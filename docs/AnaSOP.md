...
- Key readable variables or data scope: Hazard Evidence Class, Estimated Settlement Population, Accessibility Loss, Newly Isolated, Intervention Priority, Structural Rank Minimum and Maximum, Allocation-Threshold Top-10 Frequency, Weight-Rule Top-10 Frequency, and Settlement Contextual Vulnerability (Sensitivity Only).
...
- Key readable variables or data scope: Hazard Evidence Class, Estimated Settlement Population, Accessibility Loss, Settlement Contextual Vulnerability (Sensitivity Only), Priority Rank, Structural Rank Minimum and Maximum, Allocation-Threshold Top-10 Frequency, Weight-Rule Top-10 Frequency, and Population Reconnected.
- What would verify it: Priority ranks remain high across the effective structural states and persist under the population-allocation and weighting families when each source of sensitivity is reported separately.
...
\]
  ...
  where \(H_{s,c}^{*}\) is scaled hazard-evidence confidence, \(E_{s,c}^{*}\) is scaled modeled population exposure, and \(A_{s,c}^{*}\) is the isolation-preserving accessibility component. District vulnerability is excluded from the primary score. A separately labelled Appendix sensitivity score adds \(V_s^{sens}\), the shrinkage-adjusted district vulnerability context repeated across settlements, as a fourth equal-weight component. Structural scenarios, population-allocation thresholds, and alternative weighting rules are evaluated separately.
...
| Structural Rank Minimum and Maximum | Best and worst eligible rank across structural scenarios | robustness outcome | \([\min_{m\in\mathcal{M}_{s,str}}Rank_{s,m},\max_{m\in\mathcal{M}_{s,str}}Rank_{s,m}]\). | Report the observed rank range rather than treating repeated nominal factorial combinations as independent stability evidence. | yes |
...
| Family-Specific Median Priority Rank | Median rank within one structural, population-allocation, or weighting family | robustness outcome | \(\widetilde{Rank}_{s,f}=Q_{0.5}(Rank_{s,m}:m\in\mathcal{M}_{s,f})\). | Preserve the three families separately; report min-max intervals for finite structural and allocation grids and P05-P95 for random-weight draws. | yes |
...
Each family-specific frequency is defined only when its denominator is positive. The 192 nominal structural combinations collapse to ten distinct aggregate accessibility states, so their repetitions are not averaged into a composite stability score. Structural robustness is reported as paired factor activity and the observed minimum-to-maximum eligible rank; allocation robustness uses the four finite thresholds; weighting robustness uses P05-P95 intervals from the seeded draws and deterministic alternatives. The Priority-Rank Robustness Across Assumptions figure retains these families separately.
...
| 4. Establish baseline and disrupted access | Topology repair threshold (m), Settlement-to-road snap distance (m), Baseline health/emergency accessibility (minutes), Road Closure Rule, Facility Availability Rule, Post-Disruption Travel Time (minutes), Newly Isolated, Accessibility Loss (minutes) | Scenario \(c\) in Section 6.1 and shortest-path definitions in Section 4 | Road Disruption and Service-Access Loss; Structural Scenario Factor Activity | Candidate road disruptions generate finite delays or complete modeled service isolation. | Model-based support with paired checks showing which structural factors change graph or access outcomes. |
| 5. Aggregate affected population | Estimated Settlement Population, Newly Isolated Population, Population Delayed over 5 Minutes, Population-Weighted Accessibility Loss (person-minutes), Unallocated Population | Population and access-loss estimands in Section 6.6 | Structural Scenario Factor Activity | Modeled network disruption affects a non-trivial assigned population. | Supported with explicit 3 km allocation, coverage limits, and maximum paired factor effects. |
...
| 8. Test vulnerability sensitivity | Settlement Contextual Vulnerability (Sensitivity Only), Shrinkage-Adjusted District Vulnerability Percentile, Survey Reliability Category | \(\Pi_{s,c}^{V}\) in Section 6.4 | Pre-Disaster Vulnerability Profile by Supported Domain; Pre-Disaster Vulnerability Dimensions | Test whether district context changes the primary ranking in an Appendix-only sensitivity analysis. | Limited contextual sensitivity evidence; no settlement-level vulnerability inference. |
| 9. Test rank robustness | Hazard Scenario, Road Closure Rule, Facility Availability Rule, Topology repair threshold (m), Population Allocation Threshold (m), Structural Rank Minimum and Maximum, Allocation-Threshold Top-10 Frequency, Weight-Rule Top-10 Frequency | Paired factor-activity checks plus separate structural, allocation-threshold, and weighting specifications in Section 6.5 | Priority-Rank Robustness Across Assumptions; Top 10 Priority Settlements; Structural Scenario Factor Activity | The same settlements remain high priority across active structural assumptions and the two independent sensitivity families. | Supported by separate family diagnostics; no pooled stability statistic. |
...
5. Evaluate robustness from paired structural factor activity, structural rank ranges, allocation-threshold persistence, and weighting persistence separately; do not convert repeated nominal scenarios into a pooled stability probability.
...
| Priority-Rank Robustness Across Assumptions | Reports population-allocation and weighting top-10 persistence and compares family-specific rank intervals without pooling nominal scenario repetitions. | heatmap and interval plot | 2 | Allocation-Threshold Top-10 Frequency, Weight-Rule Top-10 Frequency, Structural Rank Minimum and Maximum, Family-Specific Median Priority Rank, Priority Rank, Intervention Priority | Appendix Figure A2 | done |
...
| Structural Scenario Factor Activity | Shows which pre-specified structural factors change graph state, accessibility outcomes, or top-ten ordering in paired comparisons holding all other factors fixed. | 5 | 10 | One structural factor | Factor, levels, paired groups, graph-active groups, access-active groups, maximum population and person-minute changes, top-ten set and order changes | Appendix Table A3 | done |
| Top 10 Priority Settlements | Presents the ten strongest event-specific priorities with transparent, non-pooled robustness diagnostics. | 10 | 9 | One priority settlement | Settlement label, Hazard Evidence Class within 500 m, Estimated Settlement Population, Accessibility Status, Intervention Priority, Priority Rank, Structural Rank Minimum and Maximum, Allocation-Threshold Top-10 Frequency, Weight-Rule Top-10 Frequency | Main text | done |
